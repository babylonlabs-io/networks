...
git checkout v1.0.0-rc.7-mock-mainnet
...
```shell
babylond init <moniker> --chain-id bbn-mock-1 --home <path>
```
...
network = "mainnet" # The Babylon testnet connects to the signet Bitcoin network
```
...
- `btc-config.network`: Specifies which Bitcoin network to connect to for 
   checkpointing. For Mock Mainnet, we use "mainnet" which is Bitcoin's test network.
...
```shell
tar -xvf bbn-mock-1.tar.gz -C <path>
```

Parameters:
- `bbn-mock-1.tar.gz`: Name of the compressed blockchain snapshot file
...
```shell
babylond start --chain-id bbn-mock-1 --home <path> --x-crisis-skip-assert-invariants
```
